...
export default function EditEventPage() {
    ...
    const fetchEventData = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (event.organizerId._id !== user.uid && event.organizerId !== user.uid) {
          ...
          setFormData({
            ...
            date: formattedDate,
            venue: event.venue || '',
            eventCapacity: event.eventCapacity?.toString() || event.totalTickets?.toString() || '',
            organizerName: event.organizerName || '',
            organizerContact: event.organizerContact || '',
          });

          // Load ticket types if available, otherwise use legacy data
          if (event.ticketTypes && event.ticketTypes.length > 0) {
            setTicketTypes(event.ticketTypes.map((ticket, index) => ({
              id: index + 1,
              name: ticket.name,
            ...
            // Use legacy ticketPrice and totalTickets
            setTicketTypes([{
              id: 1,
              name: 'General Admission',
              price: event.ticketPrice?.toString() || '',
              quantity: event.totalTickets?.toString() || '',
  ...
  const addTicketType = () => {
    ...
    setTicketTypes([...ticketTypes, { id: newId, name: '', price: '', quantity: '', sold: 0 }]);
  };
  ...
  const removeTicketType = (id) => {
    ...
    // Don't allow removing if tickets have been sold for this type
    if (ticketToRemove && ticketToRemove.sold > 0) {
      alert(`Cannot remove this ticket type. ${ticketToRemove.sold} tickets have already been sold.`);
      return;
    }
  ...
  const updateTicketType = (id, field, value) => {
    ...
    // Validate quantity cannot be less than sold
    if (field === 'quantity' && ticket && ticket.sold > 0) {
      const newQuantity = parseInt(value) || 0;
      if (newQuantity < ticket.sold) {
        setErrors(prev => ({
          ...prev,
          [`ticketType_${id}_quantity`]: `Cannot set quantity below ${ticket.sold} (already sold)`
        }));
        return;
      }
    }
  ...
  const validateForm = () => {
    ...
    if (!formData.eventCapacity) {
      ...
      } else if (capacity < totalSold) {
        newErrors.eventCapacity = `Cannot reduce capacity below ${totalSold} (tickets already sold)`;
      } else {
      ...
      ticketTypes.forEach((ticket) => {
        ...
        if (!ticket.quantity) {
          ...
          if (isNaN(qty) || qty < 1) {
            ...
            newErrors[`ticketType_${ticket.id}_quantity`] = `Cannot reduce below ${sold} (already sold)`;
          }
        }
      });
    }
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#605B51] mt-1">
                    Maximum attendees ({calculateTotalSold()} tickets already sold)
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#605B51]">Manage your ticket categories</p>
                </div>
              </div>
              <Button
                type="button"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        } rounded-lg focus:outline-none focus:ring-2 focus:ring-[#a855f7] transition-all`}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#605B51] font-medium">₦</span>
                        <input
                          type="number"
                          value={ticket.price}
              ...
              <div className="mt-6 p-5 bg-gradient-to-r from-[#D8D365]/10 via-[#a855f7]/10 to-[#D8D365]/10 rounded-xl border border-[#a855f7]/30">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-[#2d2a28]">
                      {calculateTotalCapacity().toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-[#605B51] font-medium mb-1">Tickets Sold</p>
                    <p className="text-2xl font-bold text-green-600">
                      {calculateTotalSold().toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-[#605B51] font-medium mb-1">Potential Revenue</p>
                    <p className="text-2xl font-bold bg-gradient-to-r from-[#D8D365] to-[#a855f7] bg-clip-text text-transparent">
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className="w-full px-4 py-3 text-[#2d2a28] bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#a855f7] transition-all"
          ...
          <div className="flex gap-4 pt-2">
            ...
            <Link href={`/dashboard/organizer/events/${eventId}`} className="flex-1">
              <button
                type="button"
                disabled={updating}
